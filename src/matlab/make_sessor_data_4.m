...
writematrix( s( 2 ), 'log4.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
for i = 1:s( 2 )
    x = floor( robot_trajectory( 1, i ) ) + normrnd( 0, 10 ) ;
    y = floor( robot_trajectory( 2, i ) ) + normrnd( 0, 10 ) ;
    q = robot_trajectory( 3, i ) + normrnd( 0, 5 ) ;
    state = [ x, y, q ];
    writematrix( state, 'log4.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
    ...
    s = size( measurement );
    writematrix( s( 2 ), 'log4.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
    writematrix( measurement, 'log4.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
end
