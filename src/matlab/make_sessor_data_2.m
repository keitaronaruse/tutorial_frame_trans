%
% @file make_sessor_data_2.m
% @author Keitaro Naruse
% @date 2024-09-09
% @brief Making sensor data small noise
%
...
writematrix( s( 2 ), 'log2.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
for i = 1:s( 2 )
    x = floor( robot_trajectory( 1, i ) );
    y = floor( robot_trajectory( 2, i ) );
    q = robot_trajectory( 3, i );
    state = [ x, y, q ];
    writematrix( state, 'log2.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
    ...
    s = size( measurement );
    writematrix( s( 2 ), 'log2.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
    writematrix( measurement, 'log2.txt', 'WriteMode', 'append', 'Delimiter', ' ' );
end
